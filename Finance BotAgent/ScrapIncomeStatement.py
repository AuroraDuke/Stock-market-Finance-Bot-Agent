...
def scrap (name):
    url = f"https://fintables.com/sirketler/{name}/finansal-tablolar/gelir-tablosu"
    ...
    for i in range(6):
        del arrtable[0]
    c = int((len(arrtable)/6)-1) # Veri setindeki satır sayısını hesaplar
    matrix_table = [[]]*c
    ...
    save_name = f"{name}_income_statement"
